...
def create_content(data):
    ...
    for entry in data:
        if entry["path"] not in excluded_links:
            formatted_path = entry["path"].replace(" ", "-")
            icon = entry.get("icon", "fluent:icons-20-filled")  # Replace 'default-icon-class' with a suitable default or an empty string

            link = dmc.Anchor(
                [DashIconify(icon=icon, height=20), entry["name"]],
                href=formatted_path,
    ...
    return dmc.ScrollArea(
        ...
        children=dmc.Stack(gap=0, children=[
            ...
            dmc.Divider(label="Upload", mt="2rem", mb="1rem", labelPosition="left", pl="1rem"),
            dmc.Anchor(children=[DashIconify(height=20, icon='material-symbols:create-new-folder-outline-sharp'), 'Upload Submission'], className='navbar-link', href='/submission'),
            dmc.Divider(label="Weekly Projects", mt="2rem", mb="1rem", labelPosition="left", pl="1rem"),
            *body[::-1],
